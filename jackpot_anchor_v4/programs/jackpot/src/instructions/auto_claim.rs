...
/// Anyone can trigger auto-claim — funds always go to the on-chain winner.
#[derive(Accounts)]
...
pub struct AutoClaim<'info> {
    ...
    /// Winner's USDC ATA — validated in handler against round.winner.
    #[account(mut)]
...
pub fn handler(ctx: Context<AutoClaim>, round_id: u64) -> Result<()> {
    ...
    let (fee, payout, vrf_reimburse, round_bump, winner_bytes) = {
        ...
        let winner_key = Pubkey::new_from_array(round.winner);

        // Verify winner_usdc_ata belongs to the actual winner
        require!(
            ctx.accounts.winner_usdc_ata.mint == usdc_mint,
            ErrorCode::InvalidUserUsdcAta
        );
        require!(
            ctx.accounts.winner_usdc_ata.owner == winner_key,
            ErrorCode::InvalidUserUsdcAta
        );
