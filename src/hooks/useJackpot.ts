...
import IDL from "../idl/jackpot.json";
import {
  USDC_MINT,
  ADMIN_PUBKEY,
  USDC_DECIMALS,
...
import { saveRoundToFirebase, getMaxArchivedRoundId } from "../lib/roundArchive";
...
export function useJackpot(): JackpotState {
  ...
  const depositMany = useCallback(
    async (legs: DepositLegInput[]): Promise<string> => {
      ...
      try {
        ...
        // Wait for round to exist — auto-create effect handles creation
        const roundPda = getRoundPda(roundId);
    ...
    try {
      ...
      try {
        ...
        if (claimedData) {
          ...
          await saveRoundToFirebase({ ...histRound, claimTx: sig });
        }
      } catch (e) {
  ...
  const claimUnclaimed = useCallback(async (unclaimedRoundId: number): Promise<string> => {
    ...
    try {
      ...
      if (!oldRoundData) throw new Error("Round not found on-chain");
      if (oldRoundData.status !== RoundStatus.Settled) throw new Error("Round already claimed or not settled");
