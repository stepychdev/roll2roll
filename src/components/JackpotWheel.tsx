...
const EASE_EXPONENT = 3.5; // must match landingEase exponent
...
// ─── Easing ──────────────────────────────────────────────
function landingEase(t: number): number {
...
function buildSegments(participants: Participant[]): SegmentData[] {
...
export function JackpotWheel({
  ...
  useEffect(() => {
    ...
    if (prev === null && winnerIndex !== null && wasSpinningRef.current && anim.phase !== 'landing') {
      ...
      const segAngle = (frozenParts[winnerIndex].usdcAmount / total) * Math.PI * 2;
