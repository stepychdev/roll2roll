...
import { ROUND_DURATION_SEC, SOLSCAN_CLUSTER_QUERY, USDC_MINT, WHEEL_RESULT_REVEAL_DELAY_MS, MIN_TOTAL_TICKETS, TICKET_UNIT, USDC_DECIMALS, ENABLE_MULTI_DEPOSIT } from '../lib/constants';
...
export function Home() {
  ...
  React.useEffect(() => {
    ...
    const openModal = () => {
      ...
      setWinnerSnapshot({
        ...
        color: w.color,
        degenSeed: jackpot.roundRandomnessHex
          ? `${jackpot.roundRandomnessHex}:${jackpot.roundId}:${w.address}:degen-v1`
          : null,
      });
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background dark:bg-[#0f1219] text-slate-900 dark:text-slate-100">
      ...
      {/* Bottom sticky bar */}
      {connected && (
        <div className="fixed bottom-3 sm:bottom-6 left-1/2 -translate-x-1/2 z-50 max-w-[calc(100vw-1.5rem)]">
          <div className="flex items-center gap-2 p-2 pr-3 pl-4 sm:pl-6 bg-slate-900/90 backdrop-blur-md rounded-full shadow-2xl border border-slate-700/50 text-white">
            <span className="text-xs sm:text-sm font-medium">
              Balance: <span className="text-green-400 font-bold">{jackpot.myUsdcBalance.toFixed(1)} USDC</span>
            </span>
          </div>
        </div>
      )}
